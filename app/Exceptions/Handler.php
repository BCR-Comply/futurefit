...
class Handler extends ExceptionHandler
{
    ...
    public function render($request, Throwable $exception)
    {
        ...
        foreach ($excludedExtensions as $extension) {
            if (strpos($request->url(), $extension) !== false) {
                return parent::render($request, $exception);
            }
        }
        ...
        if (env('APP_DEBUG') == false) {
            ...
            if ($request->route() && $request->route()->getName() === 'errors') {
                return response()->json([
                    'error' => true,
                    'message' => 'An error occurred on the error handling route.',
                ], 500);
            }

            $errlog = new ErrorLogs;
            $errlog->error_code = $this->isHttpException($exception) ? $exception->getStatusCode() : $exception->getCode();
            $errlog->section = request()->segment(2) ?? request()->segment(1);
            $errlog->url = url()->current();
            $errlog->date = date('Y-m-d');
            $errlog->time = date('H:i:s');
